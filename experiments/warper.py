...
class WarperApp(tk.Tk):
    def __init__(self):
        ...
        close_button = tk.Button(top_frame, text='X', fg='white', bg='red', command=self.close_app,
        ...
        # Create an upload frames button
        upload_button = tk.Button(center_frame, text="Upload Frames", fg='black', bg='white', font=('Helvetica', 12),
                                  bd=0, command=self.upload_frames)
        upload_button.pack()
        ...
        self.scale_slider = tk.Scale(right_frame, from_=0.1, to=2.0, resolution=0.01, orient=tk.HORIZONTAL,
                                     variable=self.scale_var, command=self.update_image_scale, bg='black', fg='white',
                                     length=400)
        ...
        # Create undo button
        self.undo_button = tk.Button(bottom_frame, text="Undo", fg='white', bg='red', font=('Helvetica', 12), bd=0,
                                     command=self.undo_last_point, state=tk.DISABLED)
        self.undo_button.pack(side=tk.RIGHT, pady=10)

        # Create warp button
        self.warp_button = tk.Button(bottom_frame, text="Warp Video", fg='white', bg='blue', font=('Helvetica', 12),
                                     bd=0, command=self.perform_perspective_transform, state=tk.DISABLED)
    ...
    def create_navigation_widgets(self, parent):
        ...
        self.prev_10_button = tk.Button(navigation_frame, text="<<", fg='black', bg='white', font=('Helvetica', 12),
                                        bd=0, command=lambda: self.navigate_frames(-10))
        self.prev_10_button.pack(side=tk.LEFT, padx=5)

        self.prev_1_button = tk.Button(navigation_frame, text="<", fg='black', bg='white', font=('Helvetica', 12), bd=0,
                                       command=lambda: self.navigate_frames(-1))
        self.prev_1_button.pack(side=tk.LEFT, padx=5)

        self.next_1_button = tk.Button(navigation_frame, text=">", fg='black', bg='white', font=('Helvetica', 12), bd=0,
                                       command=lambda: self.navigate_frames(1))
        self.next_1_button.pack(side=tk.LEFT, padx=5)

        self.next_10_button = tk.Button(navigation_frame, text=">>", fg='black', bg='white', font=('Helvetica', 12),
                                        bd=0, command=lambda: self.navigate_frames(10))
        ...
        self.go_button = tk.Button(navigation_frame, text="Go", fg='black', bg='white', font=('Helvetica', 12), bd=0,
    ...
    # Method to upload a directory of frames
    def upload_frames(self):
        dir_path = filedialog.askdirectory()
        if dir_path:
            self.frame_dir = dir_path
            self.show_first_frame()

    # Method to show the first frame in the directory
    def show_first_frame(self):
        frame_files = sorted(os.listdir(self.frame_dir))
        if frame_files:
            frame_path = os.path.join(self.frame_dir, frame_files[0])
            self.original_image = Image.open(frame_path)
    ...
    def navigate_frames(self, step):
        ...
        if 0 <= new_frame_index < len(os.listdir(self.frame_dir)):
            self.current_frame_index = new_frame_index
            frame_files = sorted(os.listdir(self.frame_dir))
            frame_path = os.path.join(self.frame_dir, frame_files[self.current_frame_index])
            self.original_image = Image.open(frame_path)
            self.update_image_scale()
        self.update_navigation_buttons()
    ...
    def update_navigation_buttons(self):
        ...
        self.next_10_button.config(
            state=tk.NORMAL if self.current_frame_index < self.total_frames - 10 else tk.DISABLED)
    ...
    def show_magnifier(self, x, y):
        ...
        crosshair_size = 10
        draw.line(
            (magnify_size / 2 - crosshair_size, magnify_size / 2, magnify_size / 2 + crosshair_size, magnify_size / 2),
            fill=crosshair_color)
        draw.line(
            (magnify_size / 2, magnify_size / 2 - crosshair_size, magnify_size / 2, magnify_size / 2 + crosshair_size),
            fill=crosshair_color)
